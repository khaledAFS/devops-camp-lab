...
def add_products(link):
    ...
    for product in produce:
        ...
        price = float(
            product.find("span", class_="dollars").get_text() + "." + product.find("span", class_="cents").get_text())
...
def add_pickups():
    ...
    h = Pickup(name="CUSTOMER ADDRESS", description="CUSTOMER ADDRESS", street_address="CUSTOMER ADDRESS",
               zipcode="HOME", state="CA")
    p = Pickup(name="Parnassus Farmers' Market (UCSF)", description="Wednesdays: 10 a.m-3 p.m.",
               street_address="505 Parnassus Avenue", zipcode="94122", state="CA")
    p2 = Pickup(name="Mission Bay Farmers' Market (UCSF in Mission Bay)", description="Wednesdays: 10:00 AM to 2:00 PM",
                street_address="550 Gene Friend Way", zipcode="94158", state="CA")
    p3 = Pickup(name="Noe Valley Farmers' Market", description="Saturdays: 8 a.m -1 p.m.",
                street_address="3861 24th St.", zipcode="94114", state="CA")
    p4 = Pickup(name="Inner Sunset Farmers' Market", description="Sundays: 9 a.m.-1 p.m.",
                street_address="1315 8th Ave", zipcode="94122", state="CA")
    p5 = Pickup(name="Inner Richmond: Clement Street Farmer's Market", description="Sundays: 9 a.m.-2 p.m.",
                street_address="200 Clement St.", zipcode="94118", state="CA")
    p6 = Pickup(name="Heart of the City Farmers' Market",
                description="Sundays: 7 a.m.-5 p.m.<br>Wednesdays: 7 a.m.-5:30 p.m.", street_address="1182 Market St.",
